...
A full list of all renepy authors ("contributors") can also be determined
from the VCS, e.g. via `git shortlog -sne`, or conveniently looked up on
[the GitHub web interface](https://github.com/DnPy/renepy/graphs/contributors).

Details on individual authorships of files can be obtained via the VCS,
e.g. via `git blame`, or the GitHub web interface.

If you wish to include a file from renepy in your project, make sure to
...
**renepy-native** source files, that is, files that were created by
_the renepy authors_, require the following one-line header, preferably in
the first line, as a comment:

    Copyright 20XX-20YY the renepy authors. See copying.md for legal info.
...
source projects, require the following, longer header:
    ...
    (Modifications|Other (data|code)|Everything else) Copyright 20XX-20YY the renepy authors.
    See copying.md for further legal info.

In addition to the renepy header, the file's original license header should
...
List of all 3rd-party files in renepy:
